			...
			if (script.mode == RagdollEditor.Mode.Colliders) {
				...
				for (int i = 0; i < colliders.Length; i++) {
					...
					if (colliders[i] == script.selectedCollider) {
						...
						if (Tools.current != Tool.Scale) {
							...
							Handles.CubeHandleCap(0, center, colliders[i].transform.rotation, size, EventType.Ignore);
							Handles.color = color;
						}
						...
						if (Handles.Button(center, Quaternion.identity, size * 0.5f, size * 0.5f, Handles.DotHandleCap)) {
				...
				for (int i = 0; i < rigidbodies.Length; i++) {
					...
					if (rigidbodies[i] == script.selectedRigidbody) {
						...
						float size = GetHandleSize(center);
						Handles.CubeHandleCap(0, center, rigidbodies[i].transform.rotation, size, EventType.Ignore); // TODO
						Handles.color = color;
					} else {
						// Selecting joints
						Vector3 center = rigidbodies[i].transform.position;
